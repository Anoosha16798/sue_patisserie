      ...
      {items.map((item, index) => (
        ...
        >
          <Link href={item.href} className="absolute inset-0">
            ...
            <div className="absolute inset-0 bg-linear-to-t from-cocoa/70 via-cocoa/10 to-transparent" />
            <div className="absolute right-0 bottom-0 left-0 p-5 text-foam">
              ...
              <p className="mt-1 text-sm text-foam/80">{item.caption}</p>
            </div>
          </Link>
        </motion.article>
      ))}
